...
<?php
	...
	if (isset($_GET['page']) && !isset($_GET['date'])) {
		$page 			= trim($_GET['page']);
		$date_unix 		= mktime(0, 0, 0, date("m")+($page*6)-7, 1, date("Y"));
		$date_unix_end 	= mktime(0, 0, 0, date("m")+($page*6)-1, 1, date("Y"));
		$date_text 		= strftime('%B %d, %Y', $date_unix);
		$date_text_end 	= strftime('%B %d, %Y', $date_unix_end);
	} elseif (isset($_GET['date'])) {
		$month = date('m', $_GET['date']);
		if ($month == '01') { 
			$year = date('y', $_GET['date']);
			$year2 = date('y', $_GET['date']);
			$month = date('m', $_GET['date'])-6;
			$month2 = date('m', $_GET['date']);
		} else {
			$year = date('y', $_GET['date']);
			$year2 = date('y', $_GET['date']);
			$month = date('m', $_GET['date'])-5;
			$month2 = date('m', $_GET['date'])+1;
		}
		$date_unix 		= mktime(0, 0, 0, $month, 1, $year);
		$date_unix_end 	= mktime(0, 0, 0, $month2, 1, $year2);
		
		$months_ago = floor((((time() - $date_unix)/60)/60)/24/(30.4167*6))-1;
		$page = $months_ago*(-1);
		...
		$date_unix 		= mktime(0, 0, 0, $month, 1, date("Y"));
		$date_unix_end 	= mktime(0, 0, 0, $month+6, 1, date("Y"));
		$date_text 		= strftime('%B %d, %Y', $date_unix);
		$date_text_end 	= strftime('%B %d, %Y', $date_unix_end);
	}
...
<div class="container">
	<div class="row">
    	<div class="col-md-6">
            <h1 class="center-text"><i class="icon-sun"></i> My Bi-Annual Report</h1>
      	</div>
     	<div class="col-md-6 center-text">
            <a href="report_bi_annual.php" class="btn btn-default pull-right"><i class="icon-calendar"></i> Today</a>
            <div class="btn-group">
                <a href="report_bi_annual.php?page=<?php echo htmlentities($page-1); ?>" class="btn btn-inverse"><i class="icon-chevron-left"></i> Prev</a>
                <a class="btn btn-inverse"><?php 
                        echo $date_text."-".$date_text_end;
                        ?></a>
                <a href="report_bi_annual.php?page=<?php echo htmlentities($page+1); ?>" class="btn btn-inverse"<?php if ($page == 0) {
                        echo " disabled";
    ...
    <div class="row">
        ...
        <?php //weekly breakdown bar graph info
			...
			$graph_interval = floor($interval_sec/6);
			$counter_weeks = 0;
			$ca_m1 	= array();
			$ca_m2 	= array();
			$ca_m3 	= array();
			...
			do {
				...
				foreach($ca_array as $request) {
					...
					if ($request_timestamp > $begin && $request_timestamp < $end) {
						switch($counter_weeks) {
							case 0:
								array_push($ca_m1, $request);
								break;
							case 1:
								array_push($ca_m2, $request);
								break;
							case 2:
								array_push($ca_m3, $request);
								break;
							case 3:
								array_push($ca_m4, $request);
								break;
							case 4:
								array_push($ca_m5, $request);
								break;
							case 5:
								array_push($ca_m6, $request);
								break;
						}
					}
				}
				foreach($p_array as $request) {
					...
					if ($request_timestamp > $begin && $request_timestamp < $end) {
						switch($counter_weeks) {
							case 0:
								array_push($p_m1, $request);
								break;
							case 1:
								array_push($p_m2, $request);
								break;
							case 2:
								array_push($p_m3, $request);
								break;
							case 3:
								array_push($p_m4, $request);
								break;
							case 4:
								array_push($p_m5, $request);
								break;
							case 5:
								array_push($p_m6, $request);
								break;
						}
					}
				}
				foreach($u_array as $request) {
					...
					if ($request_timestamp > $begin && $request_timestamp < $end) {
						switch($counter_weeks) {
							case 0:
								array_push($u_m1, $request);
								break;
							case 1:
								array_push($u_m2, $request);
								break;
							case 2:
								array_push($u_m3, $request);
								break;
							case 3:
								array_push($u_m4, $request);
								break;
							case 4:
								array_push($u_m5, $request);
								break;
							case 5:
								array_push($u_m6, $request);
								break;
						}
					}
				}
        ...
        <div class="col-md-12">
        	<div class="thumbnail">
            	<div class="center-text caption">
                	<h3>Causes Monthly Breakdown</h3>
                </div>
            ...
            <?php 	$beginning_month = date('m', $date_unix);
					if ($beginning_month = '01') {
            ...
            <script>
                ...
                datasets : [
					...
					},
					{
                        ...
                        data : [<?php 
								echo count($p_m1)+count($ca_m1)+count($u_m1).',';
								echo count($p_m2)+count($ca_m2)+count($u_m2).',';
								echo count($p_m3)+count($ca_m3)+count($u_m3).',';
								echo count($p_m4)+count($ca_m4)+count($u_m4).',';
								echo count($p_m5)+count($ca_m5)+count($u_m5).',';
								echo count($p_m6)+count($ca_m6)+count($u_m6); ?>]
                    }
                ]
                }
