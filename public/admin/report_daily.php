...
<?php
	...
	if (isset($_GET['page'])) {
		$page = trim($_GET['page']);
		$date_unix = mktime(0, 0, 0, date("m"), date("d")+$page, date("Y"));
		$date_unix_end = mktime(0, 0, 0, date("m"), date("d")+$page+1, date("Y"));
		$date_text = strftime('%A, %B %d, %Y', $date_unix);
	} elseif (isset($_GET['date'])) {
		$date_unix = $_GET['date'];
		$date_unix_end = $date_unix + 86400;
		$days = floor((((time() - $date_unix)/60)/60)/24);
		$page = $days*(-1);
		$date_text = strftime('%A, %B %d, %Y', $date_unix);
	} else {
		$page = 0;
		$date_unix = mktime(0, 0, 0, date("m"), date("d"), date("Y"));
		$date_unix_end = mktime(0, 0, 0, date("m"), date("d")+1, date("Y"));
		$date_text = date('l, F jS, Y');
	}
...
<div class="container">
	<div class="row">
    	<div class="col-md-6">
            <h1 class="center-text"><i class="icon-sun"></i> My Daily Report</h1>
      	</div>
     	<div class="col-md-6 center-text">
            <a href="report_daily.php" class="btn btn-default pull-right"><i class="icon-calendar"></i> Today</a>
            <div class="btn-group">
                <a href="report_daily.php?page=<?php echo htmlentities($page-1); ?>" class="btn btn-inverse"><i class="icon-chevron-left"></i> Prev</a>
                <a class="btn btn-inverse"><?php 
                    if ($page == 0) {
                        ...
                        ?></a>
                <a href="report_daily.php?page=<?php echo htmlentities($page+1); ?>" class="btn btn-inverse"<?php if ($page == 0) {
                        echo " disabled";
    ...
    <div class="row">
    	<div class="col-md-6 panel-group" id="requests">
        	<div class="panel panel-primary">
            	<div class="panel-heading center-text"><h4>Overdue Procedures</h4></div>
