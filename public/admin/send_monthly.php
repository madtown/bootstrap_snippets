...
foreach($unique_recipients as $id):
	...
	if($user = $userClass->find_user_by_id($id)) {
		if (isset($user['email']) && $user['email'] != "") {
			...
			$type 		= $user['email'];
			$date_unix = mktime(0, 0, 0, date("m")-1, date("d"), date("Y"));
			$body 		= "<strong>Monthly Maintenance Report:</strong></br>";
			...
			$body .= "<a href='http://192.168.0.31/public/admin/report_monthly.php?date={$date_unix}'><strong>Monthly Maintenance Report</strong></a></br></br>";
		...
		if (isset($user['id']) && $user['id'] != "") {
			...
			$inbox_type 	= 5;
			$inbox_status 	= 0;
			$inbox_text = $database->escape_value('Monthly Report');
			$notify_user 	= $database->escape_value(trim($user['id']));
			$inbox_link 	= 'report_monthly.php?date='.$date_unix;
