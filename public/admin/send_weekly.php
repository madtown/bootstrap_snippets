...
foreach($unique_recipients as $id):
	...
	if($user = $userClass->find_user_by_id($id)) {
		if (isset($user['email']) && $user['email'] != "") {
			...
			$type 		= $user['email'];
			$date_unix = mktime(0, 0, 0, date("m"), date("d")-7, date("Y"));
			$body 		= "<strong>Weekly Maintenance Report:</strong></br>";
			$body 		.= "You have received this email because you are designated as either a maintenance or lead person for a building or section. The link below will bring you to the weekly maintenance report.</br></br>";
			$body .= "<a href='http://192.168.0.31/public/admin/report_weekly.php?date={$date_unix}'><strong>Weekly Maintenance Report</strong></a></br></br>";
		...
		if (isset($user['id']) && $user['id'] != "") {
			...
			$inbox_type 	= 4;
			$inbox_status 	= 0;
			$inbox_text = $database->escape_value('Weekly Report');
			$notify_user 	= $database->escape_value(trim($user['id']));
			$inbox_link 	= 'report_weekly.php?date='.$date_unix;
